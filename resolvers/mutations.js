...
module.exports = {
  Mutation: {
    askQuestion: async (root, args, context) => {
      ...
      await newNotification.save().catch((error) => {
        throw new UserInputError(error);
      });
      await User.update(
        { _id: args.userFromId },
        { notifications: userFrom.notifications.concat(newNotification) },
        { upsert: true }
      );
      await User.update(
        { _id: args.userToId },
        { notifications: userTo.notifications.concat(newNotification) },
        { upsert: true }
      );
    ...
    answerQuestion: async (root, args, context) => {
      ...
      await Notification.updateOne(
        ...
        { answer: args.response, accepted: true },
        { upsert: true }
      );
      const notification = await Notification.findById(
        args.notificationId
      ).populate(["userFrom", "userTo", "post"]);
      return notification;
    },
    ...
    acceptNotification: async (root, args, context) => {
      ...
      if (notification.proposedContribution.length) {
        ...
        for (const i in post.skillFills) {
          newFill.push(
            post.skillFills[i] + notification.proposedContribution[i]
          );
        }
        await Post.updateOne(
          { _id: notification.post._id },
          {
            team: post.team.concat(notification.userFrom.username),
            skillFills: newFill,
          },
          { upsert: true }
        );
    ...
    addPrimarySkill: async (root, args) => {
      ...
      if (!isSkill) {
        ...
        await newSkill.save().catch((error) => console.log(error));
      ...
      await User.update(
        { _id: args.user },
        { primarySkills: user.primarySkills.concat(isSkill) },
        { upsert: true }
      );
      const updatedUser = await User.findById(args.user).populate([
        "primarySkills",
        "secondarySkills",
        "posts",
        "notifications",
    ...
    addSecondarySkill: async (root, args) => {
      ...
      await User.update(
        { _id: args.user },
        { secondarySkills: user.secondarySkills.concat(isSkill) },
        { upsert: true }
      );
      const updatedUser = await User.findById(args.user).populate([
        "primarySkills",
    ...
    addInterest: async (root, args) => {
      ...
      await User.update(
        { _id: args.user },
        { interests: user.interests.concat(args.interest) },
        { upsert: true }
      );
      const updatedUser = await User.findById(args.user).populate([
        "primarySkills",
    ...
    savePostToUser: async (root, args, context) => {
      ...
      await User.updateOne(
        { _id: args.user },
        { savedPosts: user.savedPosts.concat(postToAdd) },
        { upsert: true }
      );
    ...
    addPost: async (root, args, context) => {
      ...
      for (const skill of args.skillNames) {
        ...
        if (!isSkill) {
          ...
          await newSkill.save().catch((error) => {
      ...
      await User.updateOne(
        { _id: args.user },
        { posts: user.posts.concat(newPost) },
        { upsert: true }
      );
    ...
    deletePost: async (root, args, context) => {
      ...
      const updatedPosts = postMaker.posts.filter(
        (id) => id.toString() !== args.postId
      );
      await User.updateOne(
        { _id: postMaker._id },
        { posts: updatedPosts },
